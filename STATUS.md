...
- passive — auto-tiered listener: raw socket sniff > ARP table poll > mDNS join; subnet-filtered — src/discovery/passive.rs
...
- **Added subnet filter to sniff_raw**: extracts local subnet from selected interface (base + prefix), drops ARP sender/target and IPv4 source IPs that fall outside the local subnet. Prevents WAN IPs from being attributed to local MACs when promiscuous mode sees router-forwarded traffic.